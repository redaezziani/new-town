...
export const SideItem = ({ title, path, icon }: SideItemProps) => {
    ...
    return (
        <Link
            className={` w-full py-2 px-8 flex justify-start items-center gap-2   ${pathname == path ? ' text-slate-700  dark:text-slate-50 bg-slate-900/5 dark:bg-slate-50/5 rounded ' : ' text-slate-400'}`}
            href={path}>
            {icon}
            {title}
        </Link>
