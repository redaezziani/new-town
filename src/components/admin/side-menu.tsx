...
const SideMenu = () => {
    return (
        ...
        >
            ...
            <div className="flex  px-5  w-full  text-slate-300 gap-5 items-center justify-between">
...
export const SideItem = ({ title, path, icon }: SideItemProps) => {
    ...
    return (
        <Link
            className={` w-full py-2 px-8 flex justify-start items-center gap-2   ${pathname == path ? ' text-slate-900  dark:text-slate-50 bg-slate-900/5 dark:bg-slate-50/5 rounded ' : 'text-muted-foreground'}`}
            href={path}>
            {icon}
            {title}
        </Link>
